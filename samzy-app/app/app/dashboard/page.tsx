  ...
  return (
    <div className="mx-auto max-w-[1500px]">
      <div className="flex flex-col justify-between gap-4 md:flex-row md:items-end">
        ...
        <button className="h-11 rounded-xl bg-[#101828] px-5 text-sm font-semibold text-white">
          + New spreadsheet
        </button>
      </div>
      ...
      <section className="mt-6 grid gap-6 xl:grid-cols-[1.5fr_1fr]">
        <article className="rounded-2xl border border-[#eaecf0] bg-white p-6 shadow-sm">
          <div className="flex items-center justify-between">
            <div>
              <h2 className="font-semibold">Profit overview</h2>
              <p className="mt-1 text-sm text-[#667085]">
                Performance over the last seven days
              </p>
            </div>
            <button className="rounded-lg border border-[#d0d5dd] px-3 py-2 text-xs">
              This week
            </button>
          </div>

          <div className="mt-8 flex h-72 items-end gap-4 border-b border-l border-[#eaecf0] px-6 pb-4">
            {[38, 52, 45, 68, 62, 82, 92].map((height, index) => (
            ...
            {[
              ...
              >
                <div>
                  ...
                  <p className="mt-1 text-xs text-[#98a2b3]">Reorder suggested</p>
                </div>
        ...
        <div className="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          {activity.map(([title, detail, time]) => (
            ...
            >
              ...
              <p className="mt-2 text-xs text-[#98a2b3]">{time}</p>
            </div>
          ))}
        </div>
      </section>
